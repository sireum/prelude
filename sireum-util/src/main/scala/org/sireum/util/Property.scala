...
trait PropertyProvider {
  ...
  def removeProperty(key : Property.Key) = propertyMap.remove(key)

  def getProperty[T](key : Property.Key) = {
  ...
  def getPropertyOrElse[T](key : Property.Key, defaultValue : => T) =
    if (propertyMap.contains(key)) getProperty[T](key) else defaultValue

  def getPropertyOrElseUpdate[T](key : Property.Key, initValue : => T) = {
  ...
  def setProperty[T](key : Property.Key, value : T) = {
    val old = propertyMap.get(key)
    propertyMap(key) = value
    old match {
      case Some(v) => Some(old.asInstanceOf[T])
      case _       => None
    }
  }
}
...
object PropertyAdapter {
  def has[T](pp : PropertyProvider, propKey : Property.Key)(implicit m : Manifest[T]) =
    pp ? propKey
...
trait ImmutablePropertyProvider[Self <: ImmutablePropertyProvider[Self]] extends SelfType[Self] {
  ...
  def getProperty[T <: Immutable](key : ImmutableKey) = {
  ...
  def getPropertyOrElse[T <: Immutable](key : ImmutableKey, defaultValue : => T) =
